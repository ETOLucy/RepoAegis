...
| Official SWE-bench generation reuses RepoAegis agent nodes without remote delivery | `RepoAegisPatchAgent` reuses intake, research, planning, coding, and review; `GitSWEbenchRuntime` exports only the final diff | `tests/integration/test_swebench_runner.py` covers clean checkout, node reuse, leakage rejection, and resume | local Git fixture with a deterministic fake model | **Verified generation boundary**; official Docker grading is still pending paid calibration |
