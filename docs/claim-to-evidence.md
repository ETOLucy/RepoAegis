...
| A submitted task leaves `pending` and runs through the graph | `build_worker_runtime` and `repo-agent-worker` assemble queue-to-graph execution | component and integration tests | **2026-08-05 Compose run**: full stack started (api/worker/sandbox-runner/sandbox-daemon/postgres/opensearch all healthy); POST /v1/tasks enqueued; worker claimed via lease and materialized the pinned commit into an isolated workspace with a task branch (operation log + workspace volume evidence). Model-driven nodes (intake/planning/coding/verify/review/PR) are blocked by missing OPENAI_API_KEY | **Under validation**: deterministic lifecycle proven live; live model task remains blocked on credentials |
...
| Credential-free fake-remote end-to-end demo | workspace materialization + local bare-remote Git delivery are covered by `tests/integration/tools/test_workspace.py` | workspace and Git bare-remote tests | live Compose materialization evidence (2026-08-05) | **Under validation**: full graph demo (model nodes) requires model credentials |
