...
| Worker execution materializes before invoking a task-scoped graph | `WorkspaceGraphExecutor`, `WorkspaceAdapter`, `LangGraphExecutor` | `tests/integration/test_runtime_executor.py` | local bare remote plus real LangGraph state machine | **Partial**: orchestration proven with deterministic nodes; production nodes still bypass unified gateway for patch and verify |
...
| Duplicate delivery cannot repeat commit, push, or PR | durable log exists but these remote-write adapters are not assembled with it | none end to end | none | **Target invariant** |
