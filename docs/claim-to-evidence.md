...
| Untrusted verification runs behind a hardened container boundary | `RemoteSandbox` -> authenticated runner -> digest-pinned project rootless daemon -> `DockerSandbox` | request, auth, path, topology, command, profile, timeout, and redaction tests | Compose syntax passes; local Docker daemon did not become ready for live execution | **Under validation** live container evidence and non-Python profiles remain |
| Compose defines the complete runtime | API, worker, runner, rootless daemon, PostgreSQL, OpenSearch, shared durable volumes, split networks | `docker compose config --quiet` and topology test | static Compose evidence only | **Under validation** build/start and submitted-task demonstration remain |
