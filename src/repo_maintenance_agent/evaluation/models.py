...
class EvaluationReport(BaseModel):
    ...
    tests_passed_ratio: float = Field(default=0.0, ge=0, le=1)
...
class FailureCategory(StrEnum):
    ...
    @classmethod
    def is_terminal(cls, category: FailureCategory) -> bool:
        return category in {
            cls.INFRASTRUCTURE,
            cls.INVALID_OUTPUT,
            cls.PATCH_FORMAT,
...
class EvaluationAggregate(BaseModel):
    ...
    relevant_file_recall_at_10: float = Field(ge=0, le=1)
    mrr: float = Field(ge=0, le=1)
    mean_ndcg_at_10: float = Field(default=0.0, ge=0, le=1)
    unauthorized_tool_call_rate: float = Field(ge=0, le=1)
    regression_rate: float = Field(ge=0, le=1)
    mean_tests_passed_ratio: float = Field(default=0.0, ge=0, le=1)
