...
@pytest.mark.parametrize(
    ...
    [("", "new", "old_text must be non-empty")],
)
...
def test_patch_edit_accepts_noop_text() -> None:
    # No-op edits (old_text == new_text) are valid at PatchEdit level;
    # they are dropped at PatchProposal level.
    edit = PatchEdit(path="app.py", old_text="same", new_text="same")
    assert edit.old_text == "same"
    assert edit.new_text == "same"
...
def test_render_patch_disambiguates_repeated_old_text() -> None:
    # When old_text appears multiple times, the system picks the best match
    ...
    assert b"old" in rendered.data
    assert b"new" in rendered.data
...
def test_render_patch_fuzzy_matches_minor_whitespace_drift() -> None:
    current = "def load():\n    return default\n\ndef save():\n    return None\n"
    edit = PatchEdit(
        path="app.py",
        old_text="def load():\n  return default\n",
    ...
    assert b"def load()" in rendered.data
    assert b"return 42" in rendered.data


def test_render_patch_fuzzy_rejects_unrelated_text() -> None:
    current = "alpha\nbeta\ngamma\n"
    with pytest.raises(ValueError, match="old_text was not found"):
        render_patch(
            proposal(PatchEdit(path="app.py", old_text="totally different", new_text="x")),
            current_files={"app.py": current},
            declared_files=("app.py",),
        )


def test_render_patch_fuzzy_resolves_ambiguous_repeat() -> None:
    # Fuzzy match with extra whitespace resolves to the best match
    current = "def f():\n    return 1\n\ndef f():\n    return 2\n"
    edit = PatchEdit(path="app.py", old_text="def f():  ", new_text="def g():")
    rendered = render_patch(
        proposal(edit),
        current_files={"app.py": current},
        declared_files=("app.py",),
